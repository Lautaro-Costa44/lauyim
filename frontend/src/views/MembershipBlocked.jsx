...
// Bloqueo por cuota (Cuotas v1) y cuenta pendiente de aprobación (mismo mecanismo, otro motivo:
// spec 12.3): pantalla completa, mismo patrón que LicenseExpired. El socio
...
export default function MembershipBlocked() {
  ...
  const pending = useStore(s => s.accountPending)
  ...
  const retry = async () => {
    setBusy(true)
    try {
      if (!(await retryMembership())) toast(useStore.getState().accountPending
        ? t('Tu cuenta sigue pendiente. Acercate a recepción para que la habiliten.')
        : t('Tu cuota sigue vencida. Si ya pagaste, pedí en recepción que registren el pago.'))
    } catch {
  ...
  return (
    <div className="view membership-blocked" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '80vh', padding: 24, textAlign: 'center' }}>
      <div style={{ width: 64, height: 64, borderRadius: '50%', background: `color-mix(in srgb, var(${pending ? '--warning' : '--danger'}) 16%, transparent)`, color: `var(${pending ? '--warning' : '--danger'})`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
        <Icon name={pending ? 'clock' : 'lock'} size={32} />
      </div>
      {pending ? <>
        <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 10 }}>{t('Cuenta pendiente')}</h1>
        <p className="muted" style={{ fontSize: 15, maxWidth: 400, lineHeight: 1.5, marginBottom: 24 }}>
          {t('Tu cuenta está pendiente, acercate a recepción')}
        </p>
      </> : <>
      ...
      <div className="dim small" style={{ marginTop: 28, maxWidth: 360, lineHeight: 1.45 }}>
        {pending ? t('Lo que registres en este dispositivo se guarda y se sincroniza apenas te habiliten.')
