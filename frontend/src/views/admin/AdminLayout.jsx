...
export default function AdminLayout() {
  ...
  // Only asks whether the log is on, to decide if the Logs tab exists. Errors stay silent here:
  // the Logs section reports its own.
  const loadAuditEnabled = () => api('/api/admin/audit?limit=1&cat=').then(r => setAuditEnabled(r.enabled !== false)).catch(() => {})
  ...
  useEffect(() => { if (!user?.admin) return; loadUsers(); loadInvites(); loadPresets(); loadAttendance(); loadQrAccess(); loadAuditEnabled(); const iv = setInterval(() => { loadUsers(); loadAttendance() }, 15000); return () => clearInterval(iv) }, [user?.owner])
  ...
  return <div className="admin-shell">
    ...
    <div className="narrow admin-main">
