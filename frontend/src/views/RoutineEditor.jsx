...
export default function RoutineEditor({ routine: r, S, update, onBack, onDeleted }) {
  ...
  return <div className="narrow">
    <div className="hdr">
      <button className="iconbtn" onClick={onBack} aria-label={t('Plan')}><Icon name="chevronLeft" /></button>
      <div style={{ flex: 1, margin: '0 12px' }}>
        <input {...NO_AUTOFILL} name="app-routine-name"
          ...
          onChange={e => update(routines => {
            const rt = routines.find(x => x.id === r.id)
      ...
      <button
        ...
        onClick={() => glyphPicker(r.emoji, g => update(routines => {
          const rt = routines.find(x => x.id === r.id)
          if (rt) rt.emoji = g
        }))}
      >
