...
describe('rest timer set to Off', () => {
  beforeEach(() => { vi.useFakeTimers(); useUI.setState({ timer: null }); useStore.setState({ S: { ...useStore.getState().S, vibrateOnRest: false } }); navigator.vibrate = vi.fn() })
  ...
  it('vibrates once at rest completion when enabled', () => {
  ...
  it('does not vibrate at rest completion when disabled', () => {
    useUI.getState().startRest(1)
    vi.advanceTimersByTime(1000)
